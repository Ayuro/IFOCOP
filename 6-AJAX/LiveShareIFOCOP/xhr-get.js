window.document.addEventListener('DOMContentLoaded', () => {
  console.log('Script GET bien chargé après le chargement de la page !');
  ...
  // Fonction pour récupérer toutes les données des posts.
  const getData = (event) => {
    resultDiv.innerHTML = '';
    ...
    // Enfin, nous envoyons la requête XHR au moyen de la méthode send().
    xhr.send();
  };

  // Fonction pour récupérer un post après l'autre, de façon incrémentale
  const getOnePost = (event) => {
    resultDiv.innerHTML = '';
    const xhr = new XMLHttpRequest();
