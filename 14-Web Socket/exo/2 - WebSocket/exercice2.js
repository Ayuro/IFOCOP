...
/**
  I. Coté client : On doit pouvoir envoyer des messages au serveur et en recevoir du serveur après avoir affiché le document HTML.
  ...
  3. A la soumission du formulaire vous devez envoyer le contenu du formulaire (le message) au serveur WebSocket. Pensez à empêcher le navigateur Internet de produire une requête HTTP à la soumission du formulaire.

  4. En cas de réception d'un message provenant du serveur WebSocket, vous l'ajouterez à la suite dans la <div> identifiée.
**/

/**
  II. Coté serveur : On doit pouvoir recevoir des messages provenant de n'importe quel client et les envoyer à tous les clients connectés.
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  submitButton = document.querySelector('button[name="mysubmit"]'),
  message = document.querySelector('input[name="myMessage"]'),
  chat = document.querySelector("div#myChat"),
  webSocketClient = new WebSocket('ws://192.168.111.186:8888/', 'diwjs');
