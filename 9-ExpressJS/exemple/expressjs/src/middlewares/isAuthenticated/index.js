...
export const isAuthenticated = (req, _res, next) => {
  ...
  const userNotConnected = { error: 'invalid-auth', message: `Merci de vous connecter afin de pouvoir accéder à ce contenu.`, url: '/login' };
  const wrongUser = { error: 'invalid-auth', message: `Ce contenu est réservé à un autre utilisateur. Merci de vous connecter.`, url: '/login' };
  ...
  if (req?.session?.user) {
    ...
    if (sessionToken) {
      ...
      console.log('authenticatedUserPayload: ', JSON.stringify(authenticatedUserPayload));

      if (userIdParam) {
        console.log('userIdParam found in the url');
        console.log('authenticatedUserPayload.userId: ', authenticatedUserPayload.userId);
        console.log('ids match? ', authenticatedUserPayload.userId === userIdParam);

        if (authenticatedUserPayload.userId === userIdParam) {
          console.log('about to call next() function in isAuthenticated middleware...');
          return next();
        } else {
