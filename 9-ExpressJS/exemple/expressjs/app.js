...
// Ou bien, on choisit comme ci-dessous de créer plusieurs middlewares de fichiers statiques : un pour chaque type de fichier...
...
// app.get('/html/:file', (req, res, next) => { // /html/toto.html ==> request.params.file = toto.html
    ...
    if (password && email && firstname && lastname) {
      try {
        ...
        if (!existingUser) {
          ...
          console.log(`Début de l'insertion du nouveau compte en base de données...`);
          ...
          console.log(`Le compte a été créé avec succès: ${JSON.stringify(insertResult)}`);
      ...
      console.log('Il manque des informations nécessaires à la création du compte');
      notificationMessage = {
    ...
    if (formEmail && formPwd) {
      try {
        ...
        if (!existingUser) {
          ...
          const { email, password, isAdmin, id, firstname, lastname } = existingUser;
...
app.get('/profile/:userIdParam', isAuthenticated, (req, res, next) => {
  return res.render('profile', {
    ...
    userName: req?.session?.user ? req?.session?.user?.userName : 'utilisateur anonyme',
  });
});
...
app.use((err, req, res, next) => {
  const {url, error } = err;
  if ('invalid-auth' === error) {
    if (url) {
      return res.redirect(url);
    }
    console.log('something went wrong and about to redirect to home page in case of invalid-auth error');
    return res.redirect('/');
  }
  console.log('req.url: ', req.url);
  console.log('something went wrong and about to redirect to home page in case of other error', JSON.stringify(err));
  return res.redirect('/');
});
